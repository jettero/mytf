...
def decode_view(
    tview, with_goal=True, with_turtle=False, unravel_only=False, threshold=0.3
):
    ...
    mg = np.argmax(
        tview[GOAL]
    )  # the index in the flat reshape of tview[GOAL] with the max value
...
class EncoderTrait:
    ...
    def encode(self, min_size=None, one_hot=True, pad=None):
        ...
        # Out[1]: (16, 19)
        return encode_view(
            self.R, self.T, self.G, one_hot=one_hot, min_size=min_size, pad=pad
        )


class ScrollHeadTrait:
    def print_scroll_head(
        self, comment=tuple(), visicalc=True, visible_area_only=False
    ):
